...
from sisfact.auth.ldap_auth import normalize_username
...
class UserRepository:
    ...
    def find_by_username(self, username: str) -> SisFactUser | None:
        ...
        """
        email = username.strip().lower() if "@" in username else None
        with connect(self.config, "oracle") as conn:
            with conn.cursor() as cur:
                cur.execute(sql, username=normalized, email=email)
                row = cur.fetchone()
    ...
    def create_user(
        ...
        """Crea usuario local de autorizacion.
        ...
        sql = f"""
            INSERT INTO {USER_TABLE} (
                ...
                :username, :display_name, :email, UPPER(:role_code), UPPER(:auth_type), 'Y', :created_by
            )
