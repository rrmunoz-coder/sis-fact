...
if not exist "%NSSM%" (
  echo ERROR: falta service\nssm.exe x64. No se versiona en GitHub.
  exit /b 10
)
if not exist "%PYTHON%" (
...
"%NSSM%" set %SERVICE% AppStdout "%ROOT%logs\billing_one_service_out.log"
"%NSSM%" set %SERVICE% AppStderr "%ROOT%logs\billing_one_service_err.log"
